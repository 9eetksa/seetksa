...
do $$ declare rid uuid=current_setting('qa.rid')::uuid; begin
 if not exists(select 1 from public.work_notifications where request_id=rid and recipient=(current_setting('qa.client_claims')::jsonb->>'sub')::uuid and message like 'قبل مسؤول التواصل طلب التعديل%') then raise exception 'QA missing client routing acknowledgement'; end if;
 if not exists(select 1 from public.work_notifications where request_id=rid and recipient=(current_setting('qa.designer_claims')::jsonb->>'sub')::uuid and message like 'أحال مسؤول التواصل تعديل العميل%') then raise exception 'QA missing department notice'; end if;
 if not exists(select 1 from public.work_notifications where request_id=rid and recipient=(current_setting('qa.outsider_claims')::jsonb->>'sub')::uuid and message like '%قبول تعديل العميل وإحالته إلى القسم%') then raise exception 'QA missing specific control event'; end if;
end $$;
select 'PASS client acknowledgement department notification and control routing title' result;
