...
create or replace function public.work_settings_staff()
 ...
 return (
  ...
  from (
   select account.id,
    ...
    (select coalesce(jsonb_agg(membership.service_id order by membership.service_id),'[]'::jsonb)
     from public.work_memberships membership where membership.user_id=account.id) as services,
