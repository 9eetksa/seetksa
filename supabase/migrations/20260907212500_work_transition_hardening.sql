...
create or replace function public.work_routes(p_request uuid) returns jsonb language plpgsql security definer set search_path='' as $$
begin
 if not provision_private.work_read(p_request) then raise exception 'forbidden'; end if;
 return(select coalesce(jsonb_agg(jsonb_build_object('id',p.id,'service',s.name,'scope',case when provision_private.work_manager() or provision_private.work_coordinator() or p.assignee_id=auth.uid() then p.scope else null end,
