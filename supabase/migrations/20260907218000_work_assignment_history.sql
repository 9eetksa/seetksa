begin;
create table provision_private.work_assignment_history(event_id bigint primary key references public.work_events on delete cascade,previous_assignee uuid,next_assignee uuid,status_before text,status_after text);
revoke all on provision_private.work_assignment_history from public,anon,authenticated;
create function public.work_audit(p_request uuid) returns jsonb language plpgsql security definer set search_path='' as $$
begin
 if not provision_private.work_manager() then raise exception 'forbidden'; end if;
 return(select coalesce(jsonb_agg(to_jsonb(x)),'[]') from(select e.id,e.kind,e.note,e.created_at,coalesce(nullif(actor.display_name,''),'الإدارة أو الفريق') as actor_name,prior.display_name as previous_name,next.display_name as next_name,h.status_before,h.status_after from public.work_events e left join provision_private.work_assignment_history h on h.event_id=e.id left join public.account_profiles actor on actor.user_id=e.actor left join public.account_profiles prior on prior.user_id=h.previous_assignee left join public.account_profiles next on next.user_id=h.next_assignee where e.request_id=p_request order by e.id desc limit 200)x);
end; $$;
revoke all on function public.work_audit(uuid) from public,anon;
grant execute on function public.work_audit(uuid) to authenticated;
