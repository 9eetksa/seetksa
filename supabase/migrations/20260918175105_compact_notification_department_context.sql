 ...
 return jsonb_build_object(
  'items',(select coalesce(jsonb_agg(to_jsonb(item) order by item.created_at desc),'[]'::jsonb) from(
   select notification.id,notification.request_id,notification.message,notification.created_at,
    ...
    case when provision_private.work_role() in ('employee','admin','super_admin')
      or provision_private.work_role()='client' and service.active and service.client_visible
      then service.name else null end as department_name,
   ...
   left join public.work_events event on event.id=substring(notification.event_key from '^(?:control|event|journey):event:([0-9]{1,18}):(?:admin|coordinator|client)$')::bigint
    and event.request_id=notification.request_id
   left join public.work_parts part on part.id=event.part_id and part.request_id=notification.request_id
   left join public.work_services service on service.id=part.service_id
